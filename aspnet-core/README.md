...
* Check the `ConnectionStrings` in `appsettings.json` files under the `LuftBornTask.HttpApi.Host` and `LuftBornTask.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p c6b5c868-d1dd-430a-83fc-9ce268554d27
```

> `c6b5c868-d1dd-430a-83fc-9ce268554d27` is the password of the certificate, you can change it to any password you want.
...
Run `LuftBornTask.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `LuftBornTask.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `LuftBornTask.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
